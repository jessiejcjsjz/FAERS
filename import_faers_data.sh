...
DRUG="CREATE TABLE DRUG (ISR INT,DRUG_SEQ INT,ROLE_CODE TEXT,DRUGNAME TEXT,VAL_VBM INT,ROUTE TEXT,DOSE_VBM TEXT,DECHAL TEXT,RECHAL TEXT,LOT_NUM CHAR,EXPT_DT INT,NDA_NUM TEXT);" 
...
if [ -e "faers.db" ]; then
  echo "Do you want to overwrite existing database? y/[n]"
  read response
  if [[ "$response" == "y" ]]; then
...
sqlite3 faers.db "CREATE INDEX demo_idx ON DEMO (REPORTER_COUNTRY, EVENT_DT)"
